...
function Section({ title, icon, iconBg, children }) {
  return (
    <div className="admin-card rounded-2xl p-5 lg:p-6">
      <div className="flex items-center gap-3 mb-5">
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${iconBg}`}><i className={`fas ${icon} text-sm`}/></div>
        <h3 className="text-primary-dark font-display font-semibold">{title}</h3>
>>>>>>> frontmoneflo
      </div>
      {children}
    </div>
...
=======
  return (
    <div>
      <label className="block text-neutral-dark text-sm font-medium mb-1.5">{label}</label>
      {children}
      {hint && <p className="text-neutral text-xs mt-1.5">{hint}</p>}
    </div>
  );
}
...
=======
    <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-neutral-50 border border-neutral-light/50">
      <div><p className="text-neutral-dark text-sm font-medium">{label}</p>{desc && <p className="text-neutral text-xs mt-0.5">{desc}</p>}</div>
      <button type="button" onClick={() => onChange(!checked)}
        className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full transition-colors duration-200 ${checked ? 'bg-tertiary' : 'bg-neutral-light'}`}>
>>>>>>> frontmoneflo
...
=======
    <div className={`rounded-xl border-2 border-dashed cursor-pointer p-4 transition-all ${drag ? 'border-tertiary bg-tertiary/5' : 'border-neutral-light hover:border-tertiary/50'}`}
>>>>>>> frontmoneflo
...
=======
        <div className="w-14 h-14 rounded-xl bg-neutral-50 flex items-center justify-center flex-shrink-0 overflow-hidden border border-neutral-light">
          {currentUrl ? <img src={currentUrl} alt="p" className="w-full h-full object-contain" />
            : fallbackSrc ? <img src={fallbackSrc} alt="d" className="w-full h-full object-contain opacity-50" />
            : <i className="fas fa-image text-neutral-light text-xl" />}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-neutral-dark text-sm font-medium">{currentUrl ? 'Klik/seret untuk ganti' : 'Klik atau seret file ke sini'}</p>
          <p className="text-neutral text-xs mt-0.5">{hint}</p>
          {currentUrl && <button type="button" onClick={e => { e.stopPropagation(); onClear(); }} className="mt-1.5 text-xs text-red-500 hover:text-red-600"><i className="fas fa-times mr-1"/>Hapus (gunakan default)</button>}
>>>>>>> frontmoneflo
        </div>
      </div>
    </div>
...
=======
        <p className="text-neutral text-[11px] uppercase tracking-wider font-semibold mb-2">Halaman Login</p>
        <div className="bg-gradient-to-br from-[#083D56] to-[#00695C] rounded-xl p-4 flex items-center justify-center">
          <div className="bg-white rounded-xl p-4 w-full max-w-[200px] text-center shadow-lg">
            <div className="w-9 h-9 rounded-lg mx-auto overflow-hidden mb-1.5"><img src={ls} alt="logo" className="w-full h-full object-contain"/></div>
>>>>>>> frontmoneflo
...
=======
        <p className="text-neutral text-[11px] uppercase tracking-wider font-semibold mb-2">Sidebar Pengguna</p>
        <div className="bg-[#083D56] rounded-xl p-3.5 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0 bg-white/10"><img src={l2} alt="icon" className="w-full h-full object-contain"/></div>
>>>>>>> frontmoneflo
...
=======
          <p className="text-neutral text-[11px] uppercase tracking-wider font-semibold mb-2">Banner Pengumuman</p>
          <div className="rounded-xl bg-amber-50 border border-amber-200 px-3 py-2">
            <p className="text-amber-600 text-xs"><i className="fas fa-bullhorn mr-1.5"/>{d.announcement}</p>
>>>>>>> frontmoneflo
          </div>
        </div>
      )}
...
=======
        <p className="text-neutral text-[11px] uppercase tracking-wider font-semibold mb-2">Status Pendaftaran</p>
        <div className={`rounded-xl px-3 py-2 text-xs font-semibold flex items-center gap-2
          ${d.registOpen ? 'bg-tertiary/10 border border-tertiary/30 text-tertiary' : 'bg-red-50 border border-red-200 text-red-500'}`}>
>>>>>>> frontmoneflo
...
=======
          <h2 className="text-primary-dark font-display font-bold text-xl">Pengaturan Sistem</h2>
          <p className="text-neutral text-sm mt-0.5">Kelola identitas, branding, dan konfigurasi aplikasi</p>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          <button type="button" onClick={() => setConfirmReset(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-neutral-light text-neutral-dark hover:text-red-500 hover:border-red-300 hover:bg-red-50 text-sm font-semibold transition-all">
            <i className="fas fa-undo text-xs" /> Reset Default
          </button>
...
=======
        <div className="flex items-center gap-4 p-4 rounded-xl bg-red-50 border border-red-200">
          ...
          <p className="text-red-600 text-sm flex-1">Semua pengaturan akan dikembalikan ke <strong>default</strong>. Perubahan yang belum disimpan akan hilang.</p>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button type="button" onClick={() => setConfirmReset(false)} className="px-3 py-1.5 rounded-lg border border-neutral-light text-neutral-dark hover:bg-neutral-50 text-xs font-semibold transition-all">Batal</button>
>>>>>>> frontmoneflo
...
=======
          <Section title="Identitas Aplikasi" icon="fa-font" iconBg="bg-primary/10 text-primary">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FRow label="Nama Aplikasi" hint={`${draft.appName.length}/30 karakter`}><TInput value={draft.appName} onChange={v => set('appName', v)} placeholder="MoneFlo" maxLength={30}/></FRow>
              <FRow label="Sub-label Sidebar" hint="Teks kecil di bawah nama pada sidebar"><TInput value={draft.sidebarSub} onChange={v => set('sidebarSub', v)} placeholder="Keuangan Organisasi" maxLength={30}/></FRow>
              <FRow label="Tagline / Subjudul" hint="Muncul di halaman login & judul tab browser"><TInput value={draft.tagline} onChange={v => set('tagline', v)} placeholder="Sistem Keuangan Organisasi" maxLength={60}/></FRow>
          ...
          <Section title="Logo & Ikon" icon="fa-image" iconBg="bg-secondary/10 text-secondary">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FRow label="Logo Halaman Login" hint="PNG/SVG · Maks 2MB · Rasio 1:1">
                <UploadZone currentUrl={draft.logoUrl} fallbackSrc={defaultLogo} hint="PNG/SVG/JPG · Maks 2MB" onUpload={url => set('logoUrl', url)} onClear={() => set('logoUrl', null)}/>
              </FRow>
...
=======
          <Section title="Favicon Browser" icon="fa-globe" iconBg="bg-tertiary/10 text-tertiary">
            <FRow label="Ikon Tab Browser" hint="ICO/PNG/SVG · Rasio 1:1 · Disarankan 32×32px">
              <UploadZone currentUrl={draft.faviconUrl} fallbackSrc={null} hint="ICO/PNG/SVG · Maks 2MB" onUpload={url => set('faviconUrl', url)} onClear={() => set('faviconUrl', null)}/>
            </FRow>
            <p className="text-neutral text-xs mt-3"><i className="fas fa-info-circle mr-1"/>Favicon berubah di tab browser segera setelah disimpan.</p>
          </Section>

          <Section title="Pengumuman & Konfigurasi" icon="fa-cog" iconBg="bg-amber-50 text-amber-500">
>>>>>>> frontmoneflo
            <div className="space-y-4">
...
=======
                  rows={3} maxLength={200} className="admin-input w-full resize-none"/>
                <p className="text-neutral text-xs mt-1">{draft.announcement.length}/200 karakter</p>
              </FRow>
              <Toggle checked={draft.registOpen} onChange={v => set('registOpen', v)}
                label="Pendaftaran Organisasi Baru"
                desc="Jika dimatikan, tombol 'Daftar Organisasi' di halaman login disembunyikan"/>
...
=======
              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center"><i className="fas fa-eye text-primary text-xs"/></div>
              <h3 className="text-primary-dark font-display font-semibold text-sm">Live Preview</h3>
            </div>
            <LivePreview d={draft}/>
            <div className="mt-5 pt-4 border-t border-neutral-light/30">
              <p className="text-neutral text-[11px] text-center">Preview berubah real-time. Klik <span className="text-neutral-dark font-semibold">Simpan</span> untuk menerapkan.</p>
>>>>>>> frontmoneflo
            </div>
          </div>
        </div>
