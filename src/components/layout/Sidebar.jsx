...
export default function Sidebar({ onTambah, onOrgClick }) {
  const { currentPage, navigateTo, profile, sidebarOpen } = useApp();

  return (
    <aside className={`sidebar bg-primary ${sidebarOpen ? 'open' : ''}`}>
      {/* Logo */}
      <div className="px-6 py-5 border-b border-white/10 flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
            <img src="/logoproject2.png" alt="MoneFlo Logo" className="w-full h-full object-contain" />
          </div>
          <div>
      ...
      <div className="px-4 pb-3 flex-shrink-0">
        <button type="button" onClick={onTambah} className="btn-pulse w-full flex items-center justify-center gap-2 py-3 bg-tertiary text-white rounded-xl text-sm font-semibold hover:bg-tertiary-light transition-colors">
      ...
      <div className="px-4 pb-5 flex-shrink-0">
        <div className="org-card-clickable bg-white/10 rounded-xl p-3.5" onClick={onOrgClick}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-tertiary/40 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden">
              {profile.photo ? <img src={profile.photo} alt="logo" className="w-full h-full object-cover" /> : getInitials(profile.name)}
            </div>
            ...
            <i className="fas fa-chevron-up text-white/30 text-[10px] flex-shrink-0"></i>
          </div>
        </div>
      </div>
    </aside>
