  ...
  return (
    <div className="flex flex-wrap items-center gap-2 no-print">
      ...
      <button type="button" onClick={() => onSwitchTime('bulan')}
        className={`filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 
          ${timeFilter === 'bulan' 
            ? 'bg-primary text-white hover:bg-primary-light' 
            : 'bg-neutral-50 text-neutral-dark hover:bg-neutral-200'}`}>
        Bulan Ini
      </button>
      ...
      <div className="card-hover bg-white rounded-2xl p-5 border border-neutral-light/30">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center">
            <i className="fas fa-wallet text-primary text-sm" />
          </div>
        </div>
        <p className={`font-display font-bold text-2xl stat-number ${isSaldoNegative ? 'text-red-500' : 'text-primary-dark'}`}>
          {formatRupiah(saldo)}
        </p>
        <p className={`text-xs mt-1 ${isSaldoNegative ? 'text-red-500' : 'text-tertiary'}`}>
          <i className="fas fa-wallet mr-1" />Total Saldo Akumulatif
        </p>
      </div>
