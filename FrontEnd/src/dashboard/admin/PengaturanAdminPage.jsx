...
export default function PengaturanAdminPage() {
  ...
  return (
    <div className="page-enter space-y-5">
      {/* Header */}
      <div>
        <h2 className="text-white font-display font-bold text-xl">Akun Admin</h2>
        <p className="text-slate-400 text-sm mt-0.5">Kelola informasi akun dan keamanan administrator</p>
      </div>

      {/* Top row — Info Akun + Ubah Password */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Info Akun Admin */}
        <Section title="Informasi Akun Admin" icon="fa-user-shield" accent="indigo">
        ...
        <Section title="Ubah Kata Sandi" icon="fa-key" accent="violet">
          ...
          <div className="mt-4 pt-4 border-t border-white/5">
            <div className="space-y-2">
              {['Minimal 6 karakter', 'Kombinasi huruf & angka direkomendasikan', 'Jangan gunakan kata sandi yang sama dengan yang lama'].map(tip => (
                <div key={tip} className="flex items-start gap-2">
                  <i className="fas fa-check-circle text-indigo-400 text-xs mt-0.5" />
      ...
      {/* Bottom row — Info Sistem + Aktivitas */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* Aktivitas & Statistik */}
        <Section title="Aktivitas Sistem" icon="fa-activity" accent="violet">
          <div className="space-y-3">
            {[
              { label: 'Sesi Admin',    value: '1 aktif',     icon: 'fa-user-circle', color: 'text-indigo-400' },
              { label: 'Hari Aktif',    value: '1 hari',      icon: 'fa-calendar-check', color: 'text-emerald-400' },
              { label: 'Tanggal',       value: today,         icon: 'fa-calendar', color: 'text-slate-400' },
              { label: 'Mode',          value: 'Development', icon: 'fa-code', color: 'text-amber-400' },
            ].map(({ label, value, icon, color }) => (
              <div key={label} className="flex items-center justify-between py-2 px-3 rounded-xl bg-slate-800/50 border border-white/5">
